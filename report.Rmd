...
if(!require(rstatix)){installl.packages("rstatix",repos="http://cran.us.r-project.org")}
# if(!require(knitr)){install.packages("knitr")}
if(!require(tidyverse)){install.packages("tidyverse",repos="http://cran.us.r-project.org")}
if(!require(ampvis2)){remotes::install_github("MadsAlbertsen/ampvis2")}
if(!require(devtools)) {install.packages("devtools",repos="http://cran.us.r-project.org")}
if(!require(ggdendro)){install.packages("ggendro",repos="http://cran.us.r-project.org")}
if(!require(vegan)){install.packages("vegan",repos="http://cran.us.r-project.org")}
if(!require(pegas)){install.packages("pegas",repos="http://cran.us.r-project.org")}
...
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv") 
cycle_1@sam_data[[ioi]] <- reorder(cycle_1@sam_data[[ioi]])
...
obs_boxplot <- ggplot(obs_vals, aes_string(x=ioi,y = obs_vals$observed_otus, fill=ioi)) + geom_boxplot() + ylab("Observed Features") +theme_bw()
obs_boxplot 
...
form <- as.formula(paste0("observed_otus ~ ", ioi))
...
shannon_boxplot <- ggplot(shannon_vals, aes_string(x=ioi, y=shannon_vals$shannon, fill=ioi)) + geom_boxplot() + ylab("Shannon") + theme_bw() + xlab(ioi)
shannon_boxplot
...
form <- as.formula(paste0("shannon ~ ", ioi))
...
bray_pcoa <- qiime2R::read_qza("core-metrics-results/bray_curtis_pcoa_results.qza")
shannon<-qiime2R::read_qza("core-metrics-results/shannon_vector.qza")
...
metadata<-read_q2metadata("metadata.tsv")
jaccard <- qiime2R::read_qza("core-metrics-results/jaccard_pcoa_results.qza")
shannon<-qiime2R::read_qza("core-metrics-results/shannon_vector.qza")
shannon <- shannon$data %>% rownames_to_column("SampleID")
...
metadata<-read_q2metadata("metadata.tsv")
metadata[[ioi]] <- reorder(metadata[[ioi]])
jaccard <- qiime2R::read_qza("core-metrics-results/unweighted_unifrac_pcoa_results.qza")
shannon<-qiime2R::read_qza("core-metrics-results/shannon_vector.qza")
shannon <- shannon$data %>% rownames_to_column("SampleID")
...
metadata<-read_q2metadata("metadata.tsv")
metadata[[ioi]] <- reorder(metadata[[ioi]])
jaccard <- qiime2R::read_qza("core-metrics-results/weighted_unifrac_pcoa_results.qza")
shannon<-qiime2R::read_qza("core-metrics-results/shannon_vector.qza")
shannon <- shannon$data %>% rownames_to_column("SampleID")
...
euclidean_frame <- amp_ordinate(cycle_1_amp, filter_species = 0, type = "PCA", transform = "hellinger", print_caption = T,sample_color_by = ioi, sample_colorframe = T) 
euclidean_frame
ggsave("PCA_ordination_with_boundries.png", plot=euclidean_frame , path=outdir)
...
metadata<-read_q2metadata("metadata.tsv")
metadata[[ioi]] <- reorder(metadata[[ioi]])
w_unifrac_dist <- qiime2R::read_qza("core-metrics-results/weighted_unifrac_distance_matrix.qza")
u_unifrac_dist <- qiime2R::read_qza("core-metrics-results/unweighted_unifrac_distance_matrix.qza")
...
for(frame in split_weighted_measure){
  ...
  frame$Group2 <- reorder(as.factor(frame$Group2))
  p <- ggplot(frame, aes(x=frame$Group2, y=Distance)) + geom_violin()+ geom_boxplot(width=0.1)
  p<- p + stat_summary(fun.y=mean, geom="point", shape=23, size=2)
  p<-p + stat_summary(fun.y=median, geom="point", size=2, color="red")
  p <- p+geom_jitter(shape=16, position=position_jitter(0.2)) + xlab("Group") + ylab("Weighted Unifrac Distance") + theme_bw() + labs(title = (as.character(paste0("Distance to ",frame$Group1[1]))))
  print(p)
  ggsave(paste0("weighted_unifrac_boxplot_",frame$Group1[1],"-",frame$Group1[2],".png"), plot=p,path=outdir)
}
```
...
for(frame in split_data_frames){
  ...
  p<- p + stat_summary(fun.y=mean, geom="point", shape=23, size=2)
  p<-p + stat_summary(fun.y=median, geom="point", size=2, color="red")
  p <- p+geom_jitter(shape=16, position=position_jitter(0.2)) + xlab("Group") + ylab("Unweighted Unifrac Distance") + theme_bw() + labs(title = (as.character(paste0("Distance to ",frame$Group1[1]))))
  print(p)
  ggsave(paste0("unweighted_unifrac_boxplot_",frame$Group1[1],"-",frame$Group1[2],".png"), plot=p,path=outdir)
}
```
...
p <- ggplot(segment(ddata)) +
  ...
  geom_text(data=label(ddata),aes(x=x,y=y,label=label, hjust=0)) + 
...
cycle_1_phylum$Sample <- factor(cycle_1_phylum$Sample, levels=label(ddata)$label)
...
```{r Methods to do pairwise PERMANOVA comparisons, echo=FALSE, message=FALSE, warning=FALSE}
pairwise_permanova <- function(dat, column_of_int, dissim){
  ...
  permanova <- adonis(distance(comp, dissim) ~ grp)
  permanova <- tibble::tibble(permanova$aov.tab)
  colnames(permanova)[5] <- "r^2"
  colnames(permanova)[6] <- "p"
  permanova_stats <- data.frame(as.character(combos[i,1]), as.character(combos[i,2]), permanova[1,])
...
```{r Methods to do pairwise Unifrac PERMANOVA comparisons, echo=FALSE, message=FALSE, warning=FALSE}
pairwise_permanova_uni <- function(dat, column_of_int, weight){
  ...
  for(i in 1:dim(comb_tab)[1]){
    ...
    permanova <- adonis(tmp ~ grp)
    permanova <- tibble::tibble(permanova$aov.tab)
    colnames(permanova)[5] <- "r^2"
    colnames(permanova)[6] <- "p"
    permanova_stats <- data.frame(as.character(comb_tab[i,1]), as.character(comb_tab[i,2]), permanova[1,])
    colnames(permanova_stats)[1] <- "group1"
    colnames(permanova_stats)[2] <- "group2"
    data.frame(permanova_stats)
    p_res <- rbind(p_res, permanova_stats)
...
```{r Weighted Unifrac PERMANOVA, echo=FALSE, message=FALSE, warning=FALSE}
library(vegan)
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(sample_data(cycle_1)[[ioi]])
...
```{r Permanova results Weighted Unifrac from qiime, echo=FALSE, message=FALSE, warning=FALSE}
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(sample_data(cycle_1)[[ioi]])
...
```{r Bray Curtis PERMANOVA, echo=FALSE, message=FALSE, warning=FALSE}
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(sample_data(cycle_1)[[ioi]])
#PERMANOVA Bray Curtis
grp <- get_variable(cycle_1, ioi)
res <- adonis(distance(cycle_1, 'bray') ~ grp)
res
...
```{r Jaccard PERMANOVA, echo=FALSE, message=FALSE, warning=FALSE}
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(sample_data(cycle_1)[[ioi]])
# PERMANOVA Jaccard distance
grp <- get_variable(cycle_1, ioi)
res <- adonis(distance(cycle_1, "jaccard") ~ grp)
res
# PERMANOVA Pairwise Jaccard 
jaccard_pairwise <- pairwise_permanova(cycle_1, ioi, "jaccard")
kable(jaccard_pairwise) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
...
```{r Method to do pairwise anosim comparisons, echo=FALSE, message=FALSE, warning=FALSE}
...
w_uni_plot <- autoplot(res, notch=F) + theme_bw()+labs(title = "ANOSIM Weighted Unifrac") + xlab(ioi) + ylab("Rank Value")  + stat_pvalue_manual(w_uni_pear, label = "p.signif", y.position = max(res$dis.rank)-(.5*median(res$dis.rank)),step.increase = .03, tip.length = 0.01, guide="none") 
w_uni_plot
#Save image to disk
...
uw_uni_plot <- autoplot(res,notch = F) +theme_bw() + labs(title = "ANOSIM Unweighted Unifrac") +
  stat_pvalue_manual(uw_uni_pair, label="p.signif", y.position = max(res$dis.rank)-(.5*median(res$dis.rank)),step.increase = .03, tip.length = 0.01)+ xlab(as.character(ioi)) + ylab("Rank Value")
uw_uni_plot
#save to disk
...
bray_plot <- autoplot(ano, notch=F) + theme_bw() + labs(title ="ANOSIM Bray Curtis") + stat_pvalue_manual(bray_pair, label="p.signif", y.position = max(ano$dis.rank)-(.5*median(res$dis.rank)), step.increase = .03, tip.length = 0.01) + xlab(as.character(ioi)) + ylab("Rank Value")
bray_plot
#save to disk
...
kable(jaccard_pair) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
#Jaccard ANOSIM plot
jaccard_plot <- autoplot(ano, notch =F) + theme_bw() + labs(title = "ANOSIM Jaccard") + stat_pvalue_manual(jaccard_pair, label = "p.signif", y.position = max(ano$dis.rank)-(.5*median(res$dis.rank)), step.increase = .03, tip.length = 0.01) + xlab(as.character(ioi)) + ylab("Rank Value")
jaccard_plot
#save to disk 
...
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(as.factor(sample_data(cycle_1)[[ioi]]))
...
jaccard <- qiime2R::read_qza("core-metrics-results/jaccard_distance_matrix.qza")
# Jaccard 
...
```{r Lefse analysis Results, echo=FALSE, message=FALSE, warning=FALSE, fig.height=25, fig.width=10}
lefse_res_filenames <- list.files(pattern="*_res.png")
lefse_clado_filenames <- list.files(pattern="*_cladogram.png")
#lefse_clado_filenames <- list.files(pattern="*_better_cladogram.png")

lefse_filenames <- do.call(c, list(lefse_res_filenames, lefse_clado_filenames))
lefse_filenames <- mixedsort(lefse_filenames)
...
[1] qiime2R: Importing QIIME2 artifacts and associated data into R sessions. Jordan E Bisanz (2018) https://github.com/jbisanz/qiime2R.

[1] phyloseq: An R package for reproducible interactive analysis and graphics of microbiome census data. Paul J. McMurdie and Susan Holmes (2013) PLoS ONE 8(4):e61217. 

[1] Hadley Wickham (2021). forcats: Tools for Working with Categorical Variables (Factors). R package version 0.5.1.
https://CRAN.R-project.org/package=forcats

[1] H. Wickham. ggplot2: Elegant Graphics for Data Analysis. Springer-Verlag New York, 2016.

[2] export2graphlan.py (ver. 0.2.1 of 27 October 2018). Convert MetaPhlAn, LEfSe, and/or HUMAnN output to GraPhlAn input format. Authors: Francesco Asnicar 
(f.asnicar@unitn.it)

[2] Asnicar F, Weingart G, Tickle TL, Huttenhower C, Segata N. 2015. Compact graphical representation of phylogenetic data and metadata with GraPhlAn. PeerJ 3:e1029 https://doi.org/10.7717/peerj.1029

[3] phyloseq_to_ampvis2.R Andersen K.S. (2019), GitHub gist, https://gist.github.com/KasperSkytte/8d0ca4206a66be7ff6d76fc4ab8e66c6 

[3] ampvis2: an R package to analyse and visualise 16S rRNA amplicon  data. Andersen K.S., Kirkegaard R.H., Karst S.M., Albertsen M. (2018)  bioRxiv. doi: https://doi.org/10.1101/299537

[5] Alboukadel Kassambara (2020). rstatix: Pipe-Friendly Framework for Basic Statistical Tests. R package version 0.6.0. https://CRAN.R-project.org/package=rstatix

[5] Alboukadel Kassambara (2020). ggpubr: 'ggplot2' Based Publication Ready Plots. R package version 0.4.0.  https://CRAN.R-project.org/package=ggpubr

[6] Charles A. Martin (2017). ggConvexHull: Add a convex hull geom to ggplot2. R package version 0.1.0. http://github.com/cmartin/ggConvexHull

[6] Legendre, P. and Gallagher, E.D. (2001). Ecologically meaningful transformations for ordination of
species data. Oecologia 129, 271-280.

[7] Hadley Wickham (2007). Reshaping Data with the reshape Package. Journal of Statistical Software, 21(12), 1-20. URL http://www.jstatsoft.org/v21/i12/

[9]  Raivo Kolde (2019). pheatmap: Pretty Heatmaps. R package version 1.0.12. https://CRAN.R-project.org/package=pheatmap

[11] Andrie de Vries and Brian D. Ripley (2020). ggdendro: Create Dendrograms and Tree Diagrams Using 'ggplot2'. R package version 0.1.22. https://CRAN.R-project.org/package=ggdendro

[11] Sarkar, Deepayan (2008) Lattice: Multivariate Data Visualization with R. Springer, New York. ISBN 978-0-387-75968-5

[12] Jari Oksanen, F. Guillaume Blanchet, Michael Friendly, Roeland Kindt, Pierre Legendre, Dan McGlinn, Peter R. Minchin, R. B. O'Hara, Gavin L. Simpson, Peter Solymos, M. Henry H. Stevens, Eduard Szoecs and Helene Wagner (2020). vegan: Community Ecology Package. R package version 2.5-7. https://CRAN.R-project.org/package=vegan

[12] Kirill Müller and Hadley Wickham (2021). tibble: Simple Data Frames. R package version 3.0.6. https://CRAN.R-project.org/package=tibble

[12] Gregory R. Warnes, Ben Bolker and Thomas Lumley (2020). gtools: Various R Programming Tools. R package version 3.8.2. https://CRAN.R-project.org/package=gtools

[12] James M. Ward (2021). jamba: Jam Base Methods. R package version 0.0.61.900. http://github.com/jmw86069/jamba

[12] Hao Zhu (2020). kableExtra: Construct Complex Table with 'kable' and Pipe Syntax. R package version 1.3.1. https://CRAN.R-project.org/package=kableExtra

[12] Paradis E. 2010. pegas: an R package for population genetics with an integrated-modular approach. Bioinformatics 26: 419-420.

[13] Segata N, Izard J, Waldron L, Gevers D, Miropolsky L, Garrett WS, Huttenhower C. Metagenomic biomarker discovery and explanation. Genome Biol. 2011 Jun 24;12(6):R60. doi: 10.1186/gb-2011-12-6-r60. PMID: 21702898; PMCID: PMC3218848.

[13] Hadley Wickham, Romain François, Lionel Henry and Kirill Müller (2021). dplyr: A Grammar of Data Manipulation. R package version 1.0.4. https://CRAN.R-project.org/package=dplyr
