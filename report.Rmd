...
```
### PCA based on Euclidian Distance

```{r beta diversity plot of Euclidian Distance, echo=FALSE, message=FALSE, warning=FALSE}
library(qiime2R)
metadata<-read_q2metadata("metadata.tsv")
metadata[[ioi]] <- reorder(metadata[[ioi]])
eu_dist <- qiime2R::read_qza("core-metrics-results/euclidean_distance_results.qza")
shannon<-qiime2R::read_qza("core-metrics-results/shannon_vector.qza")
shannon <- shannon$data %>% rownames_to_column("SampleID")
eu_pcoa <- pcoa(eu_dist$data, correction="none")

eu_df <- data.frame(eu_pcoa$vectors[,1:2]) 
eu_df["SampleID"] <- rownames(eu_df)
rownames(eu_df) <- rownames('')

euclid <- eu_df %>%
  left_join(metadata) %>%
  left_join(shannon) %>%
  ggplot(aes_string(x="Axis.1", y="Axis.2", color=ioi, size="shannon_entropy"))+
  geom_point(alpha=0.5) +
  theme_q2r() +
  scale_size_continuous(name="Shannon Diversity") +
  scale_color_discrete(name=ioi)
...
euclid
ggsave("PCA_euclidain_ordination.png", plot=euclid , path=outdir)
...
### Weighted Unifrac qiime2 generated boxplots

```{r Weighted Unifrac qiime2 generated boxplots, echo=FALSE, message=FALSE, warning=FALSE}
bdiv_box_filenames <- list.files(path = "weighted-sig/", pattern="*-boxplots.png")

include_graphics(paste0("weighted-sig/",bdiv_box_filenames))


```

### Unweighted Unifrac qiime2 generated boxplots

```{r Unweighted Unifrac qiime2 generated boxplots ,echo=FALSE, message=FALSE, warning=FALSE}
bdiv_box_filenames <- list.files(path = "unweighted-sig/", pattern="*-boxplots.png")

include_graphics(paste0("unweighted-sig/",bdiv_box_filenames))

```

### r generated Weighted boxplots
...
### r generated Unweighted boxplots
...
#Weighted Unifrac
attach(sample_data(cycle_1))
cy.ano <- anosim(w_uni$data, grouping = eval(as.name(ioi)))
cy.ano$class.vec <- reorder(as.factor(cy.ano$class.vec))
summary(cy.ano)


png(paste0(outdir,"/ANOSIM_weighted_unifrac.png"))
plot(cy.ano) +
mtext(text = "ANOSIM Weighted Unifrac", adj=0)
dev.off()
...
#Unweighted Unifrac
attach(sample_data(cycle_1))
cy.ano <- anosim(un_uni$data, grouping = eval(as.name(ioi)))
cy.ano$class.vec <- reorder(as.factor(cy.ano$class.vec))
summary(cy.ano)


png(paste0(outdir,"/ANOSIM_unweighted_unifrac.png"))
plot(cy.ano) +
mtext(text = "ANOSIM unWeighted Unifrac", adj=0)
dev.off()
...
#Bray Curtis
attach(sample_data(cycle_1))
cy.ano <- anosim(bray$data, grouping = eval(as.name(ioi)))
cy.ano$class.vec <- reorder(as.factor(cy.ano$class.vec))
summary(cy.ano)


png(paste0(outdir,"/ANOSIM_bray.png"))
plot(cy.ano) +
mtext(text = "ANOSIM Bray-Curtis", adj=0)
dev.off()
...
#Jaccard
attach(sample_data(cycle_1))
cy.ano <- anosim(jaccard$data, grouping = eval(as.name(ioi)))
cy.ano$class.vec <- reorder(as.factor(cy.ano$class.vec))
summary(cy.ano)


png(paste0(outdir,"/ANOSIM_jaccard.png"))
plot(cy.ano) +
mtext(text = "ANOSIM Jaccard", adj=0)
dev.off()
...
lefse_filenames <- c("lefse_analysis.png", "image_graph_lefse.png","lefse_cladogram.png")
