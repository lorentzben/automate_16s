...
outdir_name <- "Figures"
if(!dir.exists(paste("./",outdir_name))) {dir.create(paste("./",outdir_name))}
outdir <- paste("./",outdir_name)
...
include_graphics(paste("weighted-sig/",bdiv_box_filenames))
...
include_graphics(paste("unweighted-sig/",bdiv_box_filenames))
...
for(frame in split_weighted_measure){
  ...
  p<- p + stat_summary(fun.y=mean, geom="point", shape=23, size=2)
  p<-p + stat_summary(fun.y=median, geom="point", size=2, color="red")
  p <- p+geom_jitter(shape=16, position=position_jitter(0.2)) + xlab("Group") + ylab("Weighted Unifrac Distance") + theme_bw() + labs(title = (as.character(paste("Distance to ",frame$Group1[1]))))
  print(p)
}
```
...
for(frame in split_data_frames){
  ...
  p<- p + stat_summary(fun.y=mean, geom="point", shape=23, size=2)
  p<-p + stat_summary(fun.y=median, geom="point", size=2, color="red")
  p <- p+geom_jitter(shape=16, position=position_jitter(0.2)) + xlab("Group") + ylab("Unweighted Unifrac Distance") + theme_bw() + labs(title = (as.character(paste("Distance to ",frame$Group1[1]))))
  print(p)
}
```
...
png(paste(outdir,"/ANOSIM_weighted_unifrac.png"))
plot(cy.ano) +
...
png(paste(outdir,"/ANOSIM_unweighted_unifrac.png"))
plot(cy.ano) +
...
png(paste(outdir,"/ANOSIM_bray.png"))
plot(cy.ano) +
...
png(paste(outdir,"/ANOSIM_jaccard.png"))
plot(cy.ano) +
...
w_uni <- qiime2R::read_qza("core-metrics-results/weighted_unifrac_distance_matrix.qza")
w_uni <- w_uni$data
form = as.formula(paste("w_uni ~ ", ioi))
# Weighted Unifrac
...
un_uni <- qiime2R::read_qza("core-metrics-results/unweighted_unifrac_distance_matrix.qza")
un_uni <- un_uni$data
form = as.formula(paste("un_uni ~ ", ioi))
# Unweighted Unifrac 
...
bray <- qiime2R::read_qza("core-metrics-results/bray_curtis_distance_matrix.qza")
bray <- bray$data 
form <- as.formula(paste("bray ~", ioi))
# Bray Curtis
...
form <- as.formula(paste("jaccard ~", ioi))
# Jaccard 
...
form = as.formula(paste("u_uni ~ ", ioi))
