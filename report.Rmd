...
```{r}
obs <- read.table("obs/metadata.tsv", header = TRUE)
obs_stat <- data.frame(obs[c('id','observed_features')])

obs[[ioi]] <- as.factor(as.character(obs[[ioi]]))

rstatix::kruskal_test(data = obs, formula = observed_features ~ id)
kruskal.test(obs, observed_features ~ id)

obs %>% group_by(Day)

temp <- rstatix::dunn_test(data = obs, formula = Day ~ id )
temp
