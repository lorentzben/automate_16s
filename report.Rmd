...
```{r table for evenness measurement of alpha div, message=FALSE, warning=FALSE, include=FALSE}
...
```{r stats for evenness measurement of alpha div, message=FALSE, warning=FALSE, include=FALSE}
...
```{r table for faith measurement of alpha div, message=FALSE, warning=FALSE, include=FALSE}
faith_vals = read.csv("./faith/metadata.tsv", sep='\t')
faith_vals %>% kbl() %>% kable_styling(bootstrap_options = c("striped","condensed","responsive"))
```

```{r stats for faith measurement of alpha div, message=FALSE, warning=FALSE, include=FALSE}
...
```{r table of observed species measurement of richness alpha div, message=FALSE, warning=FALSE, include=FALSE}
library(qiime2R)
...
obs_boxplot <- ggplot(obs_vals, aes_string(x=ioi,y = obs_vals$observed_features)) + geom_boxplot() + ylab("Observed Features") +theme_bw()
obs_boxplot 
...
form <- as.formula(paste("observed_features ~ ", ioi))
...
shannon_boxplot <- ggplot(shannon_vals, aes(x=shannon_vals[[ioi]], y=shannon_vals$shannon_entropy)) + geom_boxplot() + ylab("Shannon") + theme_bw() + xlab(ioi)
shannon_boxplot
...
form <- as.formula(paste("shannon_entropy ~ ", ioi))
...
obs_rarefy <- read.csv("alpha-rare/observed_otus.csv")
...
ggplot(bound, aes(x=depth, y=value, fill=sample, color=sample))+
...
ggplot(bound, aes(x=depth, y=value, color=fct_relevel(fct_reorder(ioi, as.numeric(ioi)))))+
  geom_point(data=bound_mean, mapping=aes(x=as.numeric(depth), y=as.numeric(average)),size=3) + 
