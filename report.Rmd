...
## UPGMA Plot for Weighted Unifrac Distance
...
## UPGMA Plot for Unweighted Unifrac Distance
...
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
w_unifrac <- UniFrac(cycle_1, weighted = TRUE)
meta <- sample_data(cycle_1)
adonis(w_unifrac ~ meta[[ioi]])

```
## Weighted Unifrac Permanova Pairwise

```{r Permanova results Weighted Unifrac from qiime, echo=FALSE, message=FALSE, warning=FALSE}
weighted_permanova <- read.csv("weighted-sig/permanova-pairwise.csv")
weighted_permanova <- data.frame(weighted_permanova)
weighted_permanova <- weighted_permanova[order(weighted_permanova$Group.1),]
weighted_permanova
```
...
```{r Unweighted Unifrac PERMANOVA, echo=FALSE, message=FALSE, warning=FALSE}

cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
u_unifrac <- UniFrac(cycle_1, weighted = FALSE)
meta <- sample_data(cycle_1)
adonis(u_unifrac ~ meta[[ioi]])

```

## Unweighted Unifrac Permanova Pairwise

```{r Permanova results Unweighted Unifrac from qiime, echo=FALSE, message=FALSE, warning=FALSE}
unweighted_permanova <- read.csv("unweighted-sig/permanova-pairwise.csv")
unweighted_permanova <- data.frame(unweighted_permanova)
unweighted_permanova <- unweighted_permanova[order(unweighted_permanova$Group.1),]
unweighted_permanova

adjust_pvalue(unweighted_permanova,p.col = "p.value",output.col = "tmp", method = "BH")
```
...
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
bray_dist <- qiime2R::read_qza("core-metrics-results/bray_curtis_distance_matrix.qza")
meta <- sample_data(cycle_1)
adonis(bray_dist$data ~ meta[[ioi]])
...
pairwise_anosim <- function(dat, column_of_int, dissim){
  ...
  for(item in combos){
  comp <- subset_samples(dat, eval(as.name(ioi)) %in% item)
  grp <- get_variable(comp, column_of_int)
  
  ano <- anosim(distance(comp, dissim), grp)
  ano_stats <- c(as.character(item), round(ano$statistic,6), ano$permutations, ano$dissimilarity, round(ano$signif,3))
  ...
  result <- add_significance(result, p.col = "p.adjust", output.col = "p.signif")
  return(result)
...
pairwise_anosim_uni <- function(dat, column_of_int, weight){
  ...
  result <- add_significance(result, p.col="p.adj",output.col = "p.signif")
  return(result)
...
w_uni_plot <- autoplot(res, notch=F) + theme_bw() +labs(title = "ANOSIM Weighted Unifrac")  + stat_pvalue_manual(w_uni_pear, label = "p.signif", y.position = max(res$dis.rank)-(.5*median(res$dis.rank)),step.increase = .03, tip.length = 0.01)
w_uni_plot
#Save image to disk
...
uw_uni_plot <- autoplot(res,notch = F) +theme_bw() + labs(title = "ANOSIM Unweighted Unifrac") +
  stat_pvalue_manual(uw_uni_pair, label="p.signif", y.position = max(res$dis.rank)-(.5*median(res$dis.rank)),step.increase = .03, tip.length = 0.01)
uw_uni_plot
#save to disk
...
bray_plot <- autoplot(ano, notch=F) + theme_bw() + labs(title ="ANOSIM Bray Curtis") + stat_pvalue_manual(bray_pair, label="p.signif", y.position = max(ano$dis.rank)-(.5*median(res$dis.rank)), step.increase = .03, tip.length = 0.01)
bray_plot
#save to disk
...
jaccard_pair
#Jaccard ANOSIM plot
jaccard_plot <- autoplot(ano, notch =F) + theme_bw() + labs(title = "ANOSIM Jaccard") + stat_pvalue_manual(jaccard_pair, label = "p.signif", y.position = max(ano$dis.rank)-(.5*median(res$dis.rank)), step.increase = .03, tip.length = 0.01)
jaccard_plot
#save to disk 
...
## Multi-response permutaiton procedure (MRPP)

```{r echo=FALSE, message=FALSE, warning=FALSE}
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(as.factor(sample_data(cycle_1)[[ioi]]))

w_uni <- qiime2R::read_qza("core-metrics-results/weighted_unifrac_distance_matrix.qza")
# Weighted Unifrac
print("MRPP based on Weighted Unifrac distance")
mrpp(w_uni$data, grouping = sample_data(cycle_1)[[ioi]])

un_uni <- qiime2R::read_qza("core-metrics-results/unweighted_unifrac_distance_matrix.qza")
# Unweighted Unifrac 
print("MRPP based on Unweighted Unifrac distance")
mrpp(un_uni$data, grouping = sample_data(cycle_1)[[ioi]])

bray <- qiime2R::read_qza("core-metrics-results/bray_curtis_distance_matrix.qza")
# Bray Curtis
print("MRPP based on Bray-Curtis distance")
mrpp(bray$data, grouping=sample_data(cycle_1)[[ioi]])


jaccard <- qiime2R::read_qza("core-metrics-results/jaccard_distance_matrix.qza")
# Jaccard 
print("MRPP based on Jaccard distance")
mrpp(jaccard$data, grouping = sample_data(cycle_1)[[ioi]])
...
```{r PERMANOVA testing using ADONIS, echo=FALSE, message=FALSE, warning=FALSE}
cycle_1 <- qza_to_phyloseq("table-dada2.qza","rooted-tree.qza","taxonomy.qza","metadata.tsv")
sample_data(cycle_1)[[ioi]] <- reorder(as.factor(sample_data(cycle_1)[[ioi]]))

w_uni <- qiime2R::read_qza("core-metrics-results/weighted_unifrac_distance_matrix.qza")
w_uni <- w_uni$data
form = as.formula(paste0("w_uni ~ ", ioi))
# Weighted Unifrac
print("PERMANOVA based on Weighted Unifrac Distance")
adonis(formula = form)


un_uni <- qiime2R::read_qza("core-metrics-results/unweighted_unifrac_distance_matrix.qza")
un_uni <- un_uni$data
form = as.formula(paste0("un_uni ~ ", ioi))
# Unweighted Unifrac 
print("PERMANOVA based on Unweighted Unifrac Distance")
adonis(form)

bray <- qiime2R::read_qza("core-metrics-results/bray_curtis_distance_matrix.qza")
bray <- bray$data 
form <- as.formula(paste0("bray ~", ioi))
# Bray Curtis
print("PERMANOVA based on Bray-Curtis distance")
adonis(form)


jaccard <- qiime2R::read_qza("core-metrics-results/jaccard_distance_matrix.qza")
jaccard <- jaccard$data
form <- as.formula(paste0("jaccard ~", ioi))
# Jaccard 
print("PERMANOVA based on Jaccard distance")
adonis(form)
...
form = as.formula(paste0("w_uni ~ ", ioi))
print("AMOVA based on Weighted Unifrac")
pegas::amova(formula = form)

u_uni <- qiime2R::read_qza("core-metrics-results/unweighted_unifrac_distance_matrix.qza")
u_uni <- u_uni$data
# Unweighted Unifrac

form = as.formula(paste0("u_uni ~ ", ioi))
print("AMOVA based on Unweighted Unifrac")
pegas::amova(formula = form)
...
# Bray Curtis
form = as.formula(paste0("bray ~", ioi))
print("AMOVA based on Bray Curtis")
pegas::amova(formula = form)
...
# Jaccard 
form <- as.formula(paste0("jaccard ~", ioi))
print("AMOVA based on Jaccard Distance")
pegas::amova(formula=form)
```
...
```{r making pairwise unifrac distances for pairwise comparisions}
pairwise_anosim_uni <- function(dat, column_of_int, weight){
  all_comparisons <- unique(sample_data(dat)[[column_of_int]])
  combos <- split(combn(all_comparisons,2),  col(combn(all_comparisons,2)))
  p_res <- c()
